...
console.log(props)
  const logoSmall = require('../../media/logo/ecogifts_logo_small.png');
  // const { changeId, remove } = props;
 

  const heading = props.favorites.length > 0 ? "MINA FAVORITER": "";
  // let giftsToRender = props.favorites.map(gift => {
  //   return (
  //     <li key={gift.id}>
  //     <img src={logoSmall} className="favorites-logo" alt="logo small" />
  //      <Link to={`/view/${gift.id}`} className="btn-link" id={gift.id} onClick={()=> props.changeId(gift.id)}> {gift.productName}</Link>
  //      <X onClick={()=>props.remove(gift)} size={18} />
  //     </li>
  //   );
  // });
